...
DNS tunnel demo: the rule engine raises critical `dns-tunnel` alerts from label entropy, the Python model independently flags the same host.

![NetMonitor during a DNS tunnel demo](docs/screenshot-dnstunnel.png)
...
07-network-security-monitor/
  ...
  docs/screenshot.png, docs/screenshot-dnstunnel.png
```
...
Environment: AMD Ryzen 5 8400F (6 cores, 12 threads), Windows 10 22H2, Rust stable (release profile, thin LTO), MSVC 18, .NET 10 (Release), Python 3.12. Every number comes from `scripts/bench.ps1`, run on an otherwise idle machine; re-run it to reproduce.

- Input: 1,000,000 synthetic packets written to pcap by `netmon gen` (fixed seed, so every run replays exactly the same bytes). Average frame ~545 bytes.
- Rust throughput: `netmon replay <file> --no-rules` and with the 4 built-in rules. Time is taken inside the process with `Instant` around the read-parse-track loop, so file reading is included but process start is not. pkt/s = packets / elapsed.
- C throughput: `sniff replay <file>` timed from outside with a stopwatch, so process start-up is included (the real figure is slightly higher).
- Peak memory (C, Rust): `PeakWorkingSet64` of the process, sampled every 20 ms until exit.
- 100,000 flows: the `flood` scenario uses a new source address for every packet, so the flow table fills to its limit and then evicts (894,294 evictions in the run); the peak working set is reported.
- Python: `analytics-py/scripts/bench_model.py` warms the model, then scores 40 batches of 5,000 flows each (250 of them a port scan). flows/s = 200,000 / wall time of the scoring loop; peak memory is read from inside the process (`GetProcessMemoryInfo`), because the venv `python.exe` is only a launcher.
- C# UI: the app runs the real-time port scan demo; working set is read at 15 s and 55 s (165 MB and 173 MB) plus the process peak. Flat numbers mean no leak over that window.
- Binary sizes: file sizes of the release artifacts on disk.
- Line counts: non-blank lines of source files (`.c/.h`, `.rs`, `.cs/.axaml`, `.py`), generated code and build output excluded.

Limits: single machine, single run per figure, synthetic traffic. Real captures have more small packets (ACKs), so pkt/s is the meaningful unit, not MB/s.
...
| dnstunnel (48-char base32 labels) | 5 (`dns-tunnel`, critical) | tunnel client flagged in the live demo, reason `flows` |
...
- [x] Values in this README are measured, not estimated
